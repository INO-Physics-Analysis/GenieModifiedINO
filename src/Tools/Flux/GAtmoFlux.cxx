...
bool GAtmoFlux::GenerateNext_1try(void)
{
  ...
  if(fGenWeighted) {
     ...
     weight   = 1.0;
     double flux = this->GetFlux(nu_pdg, Ev, costheta, phi);     //*GMA
     fluxweight = flux;                                          //*GMA
  }
...
//___________________________________________________________________________//*GMA
void GAtmoFlux::ForceEnergyWeight(double wt)
{
  wt = TMath::Max(-1.0, wt);
  wt = TMath::Min(5.0, wt);
  fEnergyWeight = wt;
...
//___________________________________________________________________________//*GMA
void GAtmoFlux::nu_EnergyBin(int& nbin, double* lowerbins) {
    nbin = fTotalFluxHisto->GetXaxis()->GetNbins();
    for (int ij=0; ij<=nbin; ij++) {
      lowerbins[ij] = fTotalFluxHisto->GetXaxis()->GetBinLowEdge(ij+1);
      //std::cout <<"lowerbin2d "<< ij<<" "<<lowerbins[ij]<<std::endl;
    }
}
//___________________________________________________________________________//*GMA

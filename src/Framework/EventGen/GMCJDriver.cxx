...
//_________________________//*GMA_____________________________________
double GMCJDriver::ComputeTotalEvent(int dummyloop, double& tmprateofevents, double& scaled_tmprateofevents)
{
  // attempt generating a neutrino interaction by firing a single flux neutrino
  //
  partOrTotal = 0;
  tmprateofevents=0;
  scaled_tmprateofevents=0;


  RandomGen * rnd ;
  double Pno, Psum=0;double psum_real, buffer;
  double R=0;
  bool pl_ok;
  double ebinwidth;
  ...
  //     LOG("GMCJDriver", pNOTICE)
  //       << "Computing interaction probabilities for max. path lengths";
  //     std::cout <<"comput2 "<<std::endl;
  //     Psum = this->ComputeInteractionProbabilities(true /* <- max PL*/);
  //     // psum_real=(Psum-fmod(Psum,1.))*1e-20;
  //     //Psum=fmod(Psum,1.);//-psum_real)*1e-8;

  //     buffer=Psum-int(Psum/1.)*1.;
  //     psum_real=int(Psum/1.)*1/tenpower;
  //     Psum=buffer;//-psum_real)*1e-8;
  //     Pno  = 1-Psum;
  //     LOG("GMCJDriver", pNOTICE)
  //       << "The no-interaction probability (max. path lengths) is: "
  //       << 100*Pno << " %";
  ...
  //       exit(1);
  //     }
  //     std::cout<<"Totgmcj Psum= "<<Psum<<"  psreal= "<<psum_real<<"  R= "<<R<<std::endl;
  //     if(R>=1-Pno) {
  //       LOG("GMCJDriver", pNOTICE)
  // 	<< "** Rejecting current flux neutrino";
  //       return 0;
  //     }
  //   } // preselect
  //  std::cout<< "Totxxxxgmcj_sending8 "<<int(fPreSelect)<<" "<<int(fPreSelect)<<" xx "<<dummyloop<<std::endl;

  pl_ok = false;
  int fluxpass=0;

  const int nTotnu=fNuList.size();
  int ndummybin;
  double E_nu_array[1000];
  fFluxDriver->nu_EnergyBin(ndummybin, E_nu_array);
  const int nEnergyBin = ndummybin;

  double anorm[nTotnu][nEnergyBin];
  double rateofevents_enu[nTotnu][nEnergyBin];
  double scaled_rateofevents_enu[nTotnu][nEnergyBin];

  double fr_anorm[nTotnu][nEnergyBin];
  double fr_rateofevents_enu[nTotnu][nEnergyBin];
  double fr_scaled_rateofevents_enu[nTotnu][nEnergyBin];


  for (int ij=0; ij<nTotnu; ij++) {
    for (int jk=0; jk<nEnergyBin; jk++) {
      anorm[ij][jk] = 0.0;
      rateofevents_enu[ij][jk] = 0.0;
      scaled_rateofevents_enu[ij][jk]= 0.0;

      fr_anorm[ij][jk] = 0.0;
      fr_rateofevents_enu[ij][jk] = 0.0;
      fr_scaled_rateofevents_enu[ij][jk]= 0.0;

    }
  }
  double enrweight=-fFluxDriver->AtmoEnergyWeight();
  for(int dummycount=0;dummycount<dummyloop;dummycount++) {
  ...
  if(fFluxIntTree) {
  ...
  } else {  // Else compute them in the usual manner
    // Compute (pathLength x density x weight fraction) for all materials
    // in the input geometry, for the neutrino generated by the flux driver
    bool flux_ok = this->GenerateFluxNeutrino();
    int ipdg_id = -1;
    for (unsigned int ix=0; ix<fNuList.size(); ix++) {
       if (fFluxDriver->PdgCode()==fNuList[ix]) {
          ipdg_id = ix; break;
         }
    }
    if (ipdg_id <0 || ipdg_id>=int(fNuList.size())) continue;
    ...
    double nuenergy=nup4.Energy();

    double fluxvalue= fFluxDriver->FluxWeight();
    for(int energyloop=1; energyloop<nEnergyBin; energyloop++){
  	   if(nuenergy<E_nu_array[energyloop]){
  	      ebinwidth	=	E_nu_array[energyloop]-E_nu_array[energyloop-1];

  	      rateofevents_enu[ipdg_id][energyloop] += psum_real * fluxvalue * ebinwidth * pow(nuenergy, enrweight);
  	      scaled_rateofevents_enu[ipdg_id][energyloop] += psum_real * fluxvalue * ebinwidth;
  	      anorm[ipdg_id][energyloop]+=1.0; //pow(nuenergy, enrweight);

  	      fr_rateofevents_enu[ipdg_id][energyloop] += psum_real * fluxvalue * ebinwidth * pow(nuenergy, enrweight);
  	      fr_scaled_rateofevents_enu[ipdg_id][energyloop] += psum_real * fluxvalue * ebinwidth;
  	      fr_anorm[ipdg_id][energyloop]+=1.0; //pow(nuenergy, enrweight);
    ...
    //std::cout<<"Totalgmcj dummy evt: "<<fNuList[ipdg_id]<<" "<<fluxvalue<<" "<<nup4.Energy()<<" "<<nup4.CosTheta()<<" "<<nup4.Phi()<<" "<<fFluxDriver->PdgCode()<<" "<<fluxpass<<std::endl;
    if (dummycount>0 && dummycount%100000==0) {
      double fr_tmprateofevents=0;
  	  double fr_scaled_tmprateofevents=0;
      double tot_tmprateofevents=0;
      double tot_scaled_tmprateofevents=0;

      for (int inu=0; inu<nTotnu; inu++) {
        for(int energyloop=1; energyloop<nEnergyBin; energyloop++){
            if(anorm[inu][energyloop]>0.) {
              tot_tmprateofevents += rateofevents_enu[inu][energyloop]/anorm[inu][energyloop];
              tot_scaled_tmprateofevents += scaled_rateofevents_enu[inu][energyloop]/anorm[inu][energyloop];
            }

            if (fr_anorm[inu][energyloop]>0.) {
              fr_tmprateofevents += fr_rateofevents_enu[inu][energyloop]/fr_anorm[inu][energyloop];
              fr_scaled_tmprateofevents += fr_scaled_rateofevents_enu[inu][energyloop]/fr_anorm[inu][energyloop];
            }

            //std::cout<<"Totgmcj iev "<<dummycount<<" flvour= "<<inu<<","<<fNuList[inu]<<" Ebin= "<<energyloop<<" "<<rateofevents_enu[inu][energyloop]<<" "<<scaled_rateofevents_enu[inu][energyloop]<<" "<<anorm[inu][energyloop] <<" "<<fr_rateofevents_enu[inu][energyloop]<<" "<<fr_scaled_rateofevents_enu[inu][energyloop]<<" "<<fr_anorm[inu][energyloop] <<" tot "<<tot_tmprateofevents<<" "<<tot_scaled_tmprateofevents<<" "<<fr_tmprateofevents<<" "<<fr_scaled_tmprateofevents<<std::endl;

            fr_rateofevents_enu[inu][energyloop] = fr_scaled_rateofevents_enu[inu][energyloop] = fr_anorm[inu][energyloop] = 0.0;
  	     }
  	   }
        //std::cout<<"TotgmcjFraction iev "<<dummycount<<" "<<tot_tmprateofevents<<" "<<tot_scaled_tmprateofevents<<" "<<fr_tmprateofevents<<" "<<fr_scaled_tmprateofevents<<std::endl;
      }
    }
  }	//dummycount loop ends
  ...
  for (int inu=0; inu<nTotnu; inu++) {
    for(int energyloop=1; energyloop<nEnergyBin; energyloop++){
      if(anorm[inu][energyloop]>0.) {
	       tmprateofevents += rateofevents_enu[inu][energyloop]/anorm[inu][energyloop];
	       scaled_tmprateofevents += scaled_rateofevents_enu[inu][energyloop]/anorm[inu][energyloop];
      }
      //std::cout<<"Totgmcj_enurate::::  Nu_flavour= "<<inu<<","<<fNuList[inu]<<" Ebin= "<<energyloop<<" rateofevents_enu[inu][energyloop]= "<<rateofevents_enu[inu][energyloop]<<"  anorm= "<<anorm[inu][energyloop]<<"  Int.RateofEvts= "<<tmprateofevents<<" "<<scaled_tmprateofevents<<std::endl;
    }
  }

  partOrTotal = 1;
  //return tmprateofevents;
  return 1.0;
}
//____________________________//*GMA__________________________________________
...
double GMCJDriver::ComputeInteractionProbabilities(bool use_max_path_length)
{
  ...
  //  std::cout<<"real= "<<probsum_nn<<"  nmld= "<<probsum<<std::endl;
  while(tenpower<1/probsum_nn){tenpower *=10.;} //*GMA
  //return probsum+int(probsum_nn*tenpower)*1.;
  if(partOrTotal) {
    return probsum;
  } else { 				//*GMA
    return probsum+int(probsum_nn*tenpower)*1.;

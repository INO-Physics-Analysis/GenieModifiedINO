...
//____________________________________________________________________________
void NtpWriter::Initialize2(float fEnergyWeight, int nbins, double* loweredge)		//*GMA
{

  nhistbin=nbins;
  for (int ij=0; ij<=nbins; ij++) {
    en_range[ij] = loweredge[ij];
  }
  ...
  this->CreateEventBranch();
	...
	//---------------------------------------------------------------*GMA
	this->	WriteWeight(fEnergyWeight);
		//WriteWeightHisto(fEnergyWeight);
	//---------------------------------------------------------------*GMA
...
void NtpWriter::CreateTree(void)
{
  ...
  if(fOutTree_1) delete fOutTree_1;
  ...
  fOutTree_1=new TTree("weighttree",title.str().c_str());		//*GMA
...
//-----------------------------------------------------------------------------------------------*GMA
void NtpWriter::WriteWeight(float fEnergyWeight)
{
  wt_histo=new TH1F("wt_h","Energy Weight Histogram for HAKKM flux", nhistbin, en_range);

  for(int ij=0; ij<nhistbin; ij++) {
    double fen = 0.5*(en_range[ij]+en_range[ij+1]);
    wt_histo->Fill(fen, pow(fen, -fEnergyWeight));
    //    std::cout<<"NtpWriter::WriteWeight "<<ij<<" "<<nhistbin<<" "<<fen<<" "<<pow(fen, -fEnergyWeight)<<" "<<wt_histo->GetBinContent(ij+1)<<" "<<fEnergyWeight<<std::endl;
  }

  wt_histo->Write();
  TTree::SetBranchStyle(1);

  fEventBranch_1 = fOutTree_1->Branch("iGE_EnergyWeight", &fEnergyWeight, "iGE_EnergyWeight/F");

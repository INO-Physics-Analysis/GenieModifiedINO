...
public :
 ...
 ~NtpWriter();
  ...
  void Initialize2 (float fEnergyWeight, int nbins, double* loweredge);		//ali
  void WriteWeight(float fEnergyWeight=0);		//ali
  //void WriteWeightHisto(float fEnergyWeight=0);		//ali
...
private:
  ...
  TTree *            fOutTree_1;            // < output tree					//*GMA
  TBranch *          fEventBranch_1;        // < the generated event branch 			//*GMA
  TH1F *wt_histo;										        //*GMA
  int nhistbin;
  double en_range[1000];
};

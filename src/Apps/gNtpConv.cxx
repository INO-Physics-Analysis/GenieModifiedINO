...
void ConvertToGTracker_INO	 (void);		//*GMA
...
int main(int argc, char ** argv)
{
  ...
  //-------------------------------------------------------------------------*GMA
   case (kConvFmt_nuino_rootracker)  :

  ConvertToGTracker_INO();//gOptWeight);
  break;
  //-------------------------------------------------------------------------*GMA
...
void GetCommandLineArgs(int argc, char ** argv)
{
  ...
  if( parser.OptionExists('f') ) {
    ...
    else if (fmt == "nu_INOGEN_rootracker")  { gOptOutFileFormat = kConvFmt_nuino_rootracker;      }	//*GMA
...
string DefaultOutputFile(void)
{
  ...
  else if (gOptOutFileFormat == kConvFmt_nuino_rootracker     ) { ext = "gino.root"; 	      }		//*GMA
...
int LatestFormatVersionNumber(void)
{
  ...
  else if (gOptOutFileFormat == kConvFmt_nuino_rootracker     ) return 1;		//*GMA
...
//____________________________________________________________________________________
// GENIE GHEP EVENT TREE FORMAT ->INO ROOT TRACKER FORMATS
//____________________________________________________________________________________
/*
  goto line 257 and add the following:

  case (kConvFmt_nuino_rootracker)  :

  ConvertToGTracker_INO(int weight);
  break;


  also search out all nuaance tracker definings and descriptions and write by "nuino_tracker" and add necessary ext=gino.root and comments
*/
void ConvertToGTracker_INO(void)//int weightvalue)
{
  ...
  int	tar2pid;
  int	intid;
  float	vx,	vy,	vz,	nupx,	nupy,	nupz,	nuE;
  int	npart;
  int	pid[kNPmax], unstable_pid[kNPmax];
  float	px[kNPmax],	py[kNPmax],	pz[kNPmax];
  float 	atime;
  float   eventweight, fEnergyWeight;

  //-- open the output ROOT file
  TFile fout(gOptOutFileName.c_str(), "RECREATE");

  TH1F* iGE_weighthisto;

  //-- create the output ROOT tree
  TTree * inorootracker_tree = new TTree("INOgRooTracker","GENIE event tree rootracker format");


  inorootracker_tree->Branch("iGE_nevt",           	&nevt,          "iGE_nevt/I");
  inorootracker_tree->Branch("iGE_nupid",          	&nupid,         "iGE_nupid/I");
  inorootracker_tree->Branch("iGE_tarpid",         	&tarpid,        "iGE_tarpid/I");
  inorootracker_tree->Branch("iGE_tar2pid",        	&tar2pid,       "iGE_tar2pid/I");
  inorootracker_tree->Branch("iGE_intid",          	&intid,         "iGE_intid/I");
  inorootracker_tree->Branch("iGE_vx",             	&vx,          	"iGE_vx/F");
  inorootracker_tree->Branch("iGE_vy",            	&vy,          	"iGE_vy/F");
  inorootracker_tree->Branch("iGE_vz",            	&vz,          	"iGE_vz/F");
  inorootracker_tree->Branch("iGE_nupx",           	&nupx,          "iGE_nupx/F");
  inorootracker_tree->Branch("iGE_nupy",           	&nupy,          "iGE_nupy/F");
  inorootracker_tree->Branch("iGE_nupz",           	&nupz,          "iGE_nupz/F");
  inorootracker_tree->Branch("iGE_nuE",           	&nuE,           "iGE_nuE/F");
  inorootracker_tree->Branch("iGE_eventweight",     &eventweight,   "iGE_eventweight/F");
  inorootracker_tree->Branch("iGE_atime",          	&atime,         "iGE_atime/F");
  inorootracker_tree->Branch("iGE_npart",         	&npart,       	"iGE_npart/I");
  inorootracker_tree->Branch("iGE_pid",           	pid,           	"iGE_pid[iGE_npart]/I");
  inorootracker_tree->Branch("iGE_px",            	px,           	"iGE_px[iGE_npart]/F");
  inorootracker_tree->Branch("iGE_py",            	py,            	"iGE_py[iGE_npart]/F");
  inorootracker_tree->Branch("iGE_pz",            	pz,           	"iGE_pz[iGE_npart]/F");
  inorootracker_tree->Branch("iGE_unstable_pid",    unstable_pid,   "iGE_unstable_pid[iGE_npart]/I");  //unstable pids
  ...
  //reading weight histo from ghep to inoroot. 		//*GMA

  iGE_weighthisto= (TH1F*)fin.Get("wt_h");
  iGE_weighthisto->SetName("iGE_Weight_Histogram");

  const int nbins =iGE_weighthisto->GetXaxis()->GetNbins()+1;
  double fenbin[nbins],fen[nbins];
  for (int ij=0; ij<nbins; ij++) {
    fenbin[ij] = iGE_weighthisto->GetXaxis()->GetBinLowEdge(ij+1);
    fen[ij] = iGE_weighthisto->GetXaxis()->GetBinCenter(ij+1);
  }

  fout.cd();
  //  iGE_weighthisto = (TH1F*) tmphist->Clone();
  iGE_weighthisto->Write();
  //
  //  float  abc=-1;;
  TTree *           wttree = 0;

  wttree = (TTree*)           ( fin.Get("weighttree")  );

  wttree->SetBranchAddress("iGE_EnergyWeight", &fEnergyWeight);
  wttree->GetEntry(0);

  //   std:: cout<<"entries="<<wttree->GetEntries()<<"\tWEIGHHT="<<fEnergyWeight<<" "<<abc<<std::endl;
  TTree *           tree = 0;
  NtpMCTreeHeader * thdr = 0;
  tree = dynamic_cast <TTree *>           ( fin.Get("gtree")  );
  thdr = dynamic_cast <NtpMCTreeHeader *> ( fin.Get("header") );
  ...
  /*								//========*GMa commented
    #ifdef __GENIE_FLUX_DRIVERS_ENABLED__
    flux::GJPARCNuFluxPassThroughInfo * flux_info = 0;
    tree->SetBranchAddress("flux", &flux_info);std::cout<<"3113\n";
    #else
    LOG("gntpc", pWARN)
    << "\n Flux drivers are not enabled."
    << "\n No flux pass-through information will be written-out in the rootracker file"
  ...
  */
  //-- open the output stream
  //ofstream output(gOptOutFileName.c_str(), ios::out);		//======*GMA commented
  ...
  for(Long64_t iev = 0; iev < nmax; iev++) {
    ...
    // -- Add tracker begin tag
    //
    //output << "$ begin" << endl;		//============*GMA : all "output"->"//output"

    //
    // -- Add the appropriate reaction code
    //

    // add INO event type:	//==================================*GMA

    if(gOptOutFileFormat == kConvFmt_nuino_rootracker) {
      int evtype = utils::ghep::NuanceReactionCode(&event);
      intid=evtype;
      LOG("gntpc", pNOTICE) << "NUANCE/INO-like event type = " << evtype;
      //output << "$ nuance " << evtype << endl;
    } // nuance code
    ...
    //==================================================================*GMA
    vx= event.Vertex()->X()	;/// units:: mm;
      vy= event.Vertex()->Y()	;/// units:: mm;
	vz= event.Vertex()->Z()	;/// units:: mm;
	  atime= event.Vertex()->T()	;/// units:: ns;

	    vx=vx*1000.;		//metre to mm;
	    vy=vy*1000.;		//metre to mm;
	    vz=vz*1000.;		//metre to mm;
	    atime=atime*1e9;	//second to nanosecond;

	    //float ch=2/units::mm,ch2=2/units::ns;
	    //std::cout<<"vx))="<<event.Vertex()->X()<<" vx="<<vx<<"\t"<<vx/units::cm<<"\ttime.>>"<< event.Vertex()->T()<<" t="<<atime<<"\t"<<atime/units::s<<std::endl;
	    //std::cout<<"2*="<<ch<<"\t"<<ch/units::cm<<"\ttime"<<ch2<<"\t"<<ch2/units::s<<std::endl;
	    //==================================================================*GMA
	    //
	    // -- Add '$track' lines
	    //
		...
		int          ghep_pdgc   = p->Pdg();
		//	   							//=========*GMA
		if(iparticle==0) 	nupid	=p->Pdg();		//=========*GMA
		else if(iparticle==1)	tarpid= p->Pdg();
		else	{	pid[iparticle-1] =p->Pdg();
		}
		...
		for(vector<int>::const_iterator ip = tracks.begin(); ip != tracks.end(); ++ip)
		  {
		    ...
		    // Get particle's energy & momentum
		    /*         TLorentzVector * p4 = p->P4();				//ali commented
			       double E  = p4->Energy() / units::MeV;
			       double Px = p4->Px()     / units::MeV;
			       double Py = p4->Py()     / units::MeV;
			       double Pz = p4->Pz()     / units::MeV;
		    ...
		    if(iparticle==0) 	{nupid	=p->Pdg();
		      nupx = p4->Px()     / units::MeV;
		      nupy = p4->Py()     / units::MeV;
		      nupz = p4->Pz()     / units::MeV;
		      nuE  = p4->Energy() / units::GeV;

		      //std::cout<<"nu_energy>> "<<p4->Px()<<" "<<p4->Py()<<" "<<p4->Pz()<<" nue="<<nuE<<"GeV\tnupx>> "<<p4->Energy()<<" nupx="<<nupx<<"MeV"<<std::endl;
		      std::cout<<"nu_pid "<<p->Pdg();
		      eventweight = 0.0;
		      for(int ij=0;ij<nbins;ij++){
			//std::cout<<"lowerbinlimit="<<fenbin[ij]<<std::endl;
			if(nuE>fenbin[ij] && nuE<fenbin[ij+1]){eventweight=pow(fen[ij],-fEnergyWeight);
			  //std::cout<<"$$$ fEnergyWeight="<<fEnergyWeight<<"\teventweight="<<eventweight<<std::endl;
			  break;}

		      }
		    }
		    //else if(iparticle==1)	{tarpid= p->Pdg()-1e9;std::cout<<";  tarpid "<<p->Pdg();}
		    //else if (iparticle==2)  {tar2pid = p->Pdg();std::cout<<";  tar2pid "<<p->Pdg()<<std::endl;}

		    else if(iparticle==1)	{if (p->Pdg()>1e9) {
			tarpid= p->Pdg()-1e9;std::cout<<"ei_tarpid="<<tarpid<<";  tarpid "<<p->Pdg();} else {
						   tar2pid = p->Pdg();std::cout<<"ei_tar2pid="<<tar2pid<<";  itar2pid "<<p->Pdg()<<std::endl;
						 }
		    }
		    else if (iparticle==2 && tar2pid!=0)  {tar2pid = p->Pdg();std::cout<<"fi_tar2pid="<<tar2pid<<";  tar2pid "<<p->Pdg()<<std::endl;}


		    if (iloop==1 && npart<kNPmax)	{
		      pid[npart] =p->Pdg();



		      px[npart] = p4->Px()     / units::MeV;
		      py[npart] = p4->Py()     / units::MeV;
		      pz[npart] = p4->Pz()     / units::MeV;

		      unstable_pid[npart] =0;
		      npart++;
		      std::cout<<"     stablepid "<<p->Pdg()<<std::endl;
		    } else
		      //	/*
		      if (iloop!=1 && npart<kNPmax)	{
			unstable_pid[npart] =p->Pdg();
			...
			pid[npart] =0;
			npart++;
			std::cout<<"     unstablepid "<<p->Pdg()<<std::endl;
		      }else
			//	*/

			{
			  //std::cout<<"Number of paricles are more than "<<kNPmax <<std::endl;
			}
		    //=======================================================================*GMA
	    ...
	    /*									//ali commented
	      ...
	      #endif
	    */
	    //    }//fmt==kConvFmt_t2k_tracker							// ali commented
	    ...
	    nuE=nuE*1000.;
	    inorootracker_tree->Fill();
	    mcrec->Clear();
  } // event loop

  // output.close();					======*ali commented
  fin.Close();

...
double 		      gOptWeight=0;			             // weight of a neutrino event- energy^gOptWeight: applied to the flux value 	//*GMA
double          radius_rt= 27;                 //in metre  //*GMA
double          expotime;		                   //*GMA
//double rateofevts;                       		 //*GMA
...
int main(int argc, char** argv)
{
  ...
  flux_driver->nu_EnergyBin(ndummybin, E_nu_array);
  //  std::cout<<"ntpw.Initialize2(gOptWeight "<<gOptWeight<<std::endl;
  ntpw.Initialize2(gOptWeight,  ndummybin, E_nu_array);		//*GMA
  ...
  if (gOptKtonYrExposure>0) {
    expotime=gOptKtonYrExposure*365.25*24*3600;
    double totalevt=0;
    double scaled_totalevt=0;
    double dummy = mcj_driver->ComputeTotalEvent(1000000, totalevt, scaled_totalevt);
    double pival = acos(-1.);
    gOptNev=scaled_totalevt*expotime*4*pival*(pival*radius_rt*radius_rt);  //For the area of the circle of radius 27m
  /*    std::cout<<"\n\t\t#::#::#::#::#::#:: rateofevents= \tTime= "<<expotime
  	     <<"\tNo. of evts= "
  	     <<gOptNev<<" "
  	     <<totalevt*4*pival*(pival*radius_rt*radius_rt)<<" "
  	     <<scaled_totalevt*4*pival*(pival*radius_rt*radius_rt)<<std::endl;	//ali*
  */
      //    if ( totalevt>-1) {exit(1);}
      //    if ( totalevt>-1) {gOptNev = 5;}
  }
  // /*   ************************ //*GMA :- *******************************************
...
GFluxI* GetFlux(void)
{
  ...
  // set min/max energy:
  // transfer weight factor					//*GMA
  atmo_flux_driver->ForceEnergyWeight(gOptWeight);      //*GMA
  ...
  atmo_flux_driver->SetRadii(radius_rt, radius_rt);     //*GMA
...
// Get the command line arguments
  ...
  if( parser.OptionExists('w') ) {
    LOG("gevgen_atmo", pDEBUG) << "Setting Weight";
    gOptWeight= parser.ArgAsDouble('w');
  }
